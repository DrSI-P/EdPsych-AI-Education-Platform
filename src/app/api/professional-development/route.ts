...
async function handleEnrollment(body) {
  try {
    ...
    const existingEnrollment = await (prisma as any).enrolment.findFirst({
      where: {
        userId,
        courseId,
      },
    ...
    if (existingEnrollment) {
      return NextResponse.json(
        { message: 'Already enroled in this course' },
        { status: 200 }
      );
    }

    // Create new enrolment
    const enrolment = await (prisma as any).enrolment.create({
      data: {
        userId,
        courseId,
        startDate: new Date(),
    ...
    return NextResponse.json(
      { message: 'Successfully enroled', enrolment },
      { status: 201 }
    );
  } catch (error) {
...
async function handleProgressUpdate(body) {
  try {
    ...
    await (prisma as any).enrolment.updateMany({
      where: {
        userId,
        courseId,
      },
...
async function handleCompletion(body) {
  try {
    ...
    const updatedEnrollment = await (prisma as any).enrolment.updateMany({
      where: {
        userId,
        courseId,
      },
...
async function getUserEnrollments(userId: string, courseId?: string | null) {
  const enrollments = await (prisma as any).enrolment.findMany({
    where: {
      userId,
      ...(courseId ? { courseId } : {}),
    },
