...
import { initHeyGenService } from '@/lib/heygen/init-heygen-service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Initialize HeyGen service if not already initialized
    const heygenService = getHeyGenService();
    if (!heygenService) {
      const initialized = await initHeyGenService();
      if (!initialized) {
        return NextResponse.json(
          { error: 'Failed to initialize HeyGen service' },
          { status: 500 }
        );
      }
    }
    
    // Initialize HeyGen API for this request
