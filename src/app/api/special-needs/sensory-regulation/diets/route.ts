...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (prisma: PrismaClient) => {
      ...
      // Create timetable items
      if (validatedData.timetable && validatedData.timetable.length > 0) {
        await prisma.sensoryDietSchedule.createMany({
          data: validatedData.timetable.map(item => ({
            dietId: diet.id,
            time: item.time,
      ...
      // Return created diet with timetable
