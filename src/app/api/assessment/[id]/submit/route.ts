...
export async function POST(
  ...
  try {
    ...
    for (const answer of answers) {
      ...
      // Mark the answer based on question type
      switch (question.type) {
        ...
        case 'open-ended':
          ...
          if (question.expectedAnswer && typeof content === 'string' && content.trim()) {
            try {
              ...
              // Fall back to manual marking (marked as needing review)
        ...
        default:
          isCorrect = false; // Use false instead of null for manual marking
          feedback = 'This question type requires manual marking.';
      }
    ...
    return NextResponse.json(
      { error: 'An error occurred whilst submitting the assessment' },
      { status: 500 }
    );
  }
}
