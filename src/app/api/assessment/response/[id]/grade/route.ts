...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'An error occurred whilst fetching the response' },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    // Check if user has permission to mark (admin, teacher, professional)
    const isAdmin = session.user.role === 'admin';
    const isTeacher = session.user.role === 'teacher';
    const isProfessional = session.user.role === 'professional';
    ...
    // Update each answer with the provided mark
    for (const mark of grades as Mark[]) {
      if (!mark.answerId) continue;
    ...
    return NextResponse.json(
      { error: 'An error occurred whilst updating the grades' },
      { status: 500 }
    );
  }
}
