...
const mockConversations = [
  ...
  },
  {
    ...
    lastMessage: {
      id: 'm4-1',
      senderId: 't4',
      content: 'I\'d like to timetable a meeting to discuss Emma\'s progress and some strategies we can implement to support her learning style. Would you be available next Tuesday at 4pm?',
...
export default function EnhancedCommunicationPage() {
  ...
  // Get teacher by ID
  const getTeacher = (teacherId): void => {
    return mockTeachers.find(teacher => teacher.id === teacherId);
  };
  
  // Get child name by ID
  const getChildName = (childId): void => {
  ...
  const formatMessageTime = (timestamp): void => {
  ...
  const markConversationAsRead = (conversationId): void => {
    const updatedConversations = conversations.map(conversation => {
  ...
  const handleSelectConversation = (conversation): void => {
  ...
  const renderAttachment = (attachment): void => {
    const getIcon = (type): void => {
      switch (type) {
        case 'pdf':
  ...
  const getPriorityColor = (priority): void => {
    switch (priority) {
      case 'high':
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <input 
                ...
                onChange={(e: any) => setShowAllChildren(e.target.checked)}
                ...
                {filteredMeetings.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground max-w-md mx-auto">
                      You don't have any meetings scheduled at the moment. Use the "Timetable Meeting" button to arrange a meeting with your child's educators.
                    </p>
                  </div>
                )}
              </div>
