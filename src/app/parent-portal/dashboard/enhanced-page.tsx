...
export default function EnhancedParentDashboardPage() {
  ...
  const toggleWidgetVisibility = (widgetId): void => {
  ...
  // Change widget size
  const changeWidgetSize = (widgetId, size): void => {
    setWidgets(widgets.map(widget => 
  ...
  const resetWidgetConfiguration = (): void => {
  ...
  const getNotificationIcon = (type, priority): void => {
    switch (type) {
      case 'assessment':
  ...
  const renderWidget = (widget): void => {
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setIsCustomizing(!isCustomizing)}>
            ...
            {isCustomizing ? 'Done Customising' : 'Customise Dashboard'}
          </Button>
