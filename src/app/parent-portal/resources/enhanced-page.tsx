...
export default function EnhancedResourcesPage() {
  ...
  const getLearningStyleGuide = (): void => {
  ...
  const toggleBookmark = (resourceId): void => {
  ...
  const getCategoryName = (categoryId): void => {
  ...
  const getResourceFormatIcon = (format): void => {
    switch (format) {
      case 'pdf':
        return <FileText className="h-4 w-4" />;
      case 'mp3':
  ...
  const formatLearningStyle = (style): void => {
