...
const CurriculumDifferentiationEngine = dynamic(
  () => import('@/components/ai/curriculum-differentiation/curriculum-differentiation-engine')
);
...
function CurriculumDifferentiationContent() {
  ...
  const fetchCurriculumPlan = async (id: string) => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/curriculum/plans/${id}`, { cache: "no-store" });
      if (response.ok) {
...
// Original component
function CurriculumDifferentiationPage() {
  return (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p>
                    All differentiated content maintains alignment with UK curriculum standards and uses UK English spelling and terminology. The system ensures that whilst the presentation and approach may be adapted, the core learning objectives and curriculum requirements are preserved.
                  </p>
                </div>
              </CardContent>
            </Card>
...
const CurriculumDifferentiationPageWrapper = dynamic(
  () => Promise.resolve(CurriculumDifferentiationPage),
  { 
    ssr: false,
    loading: () => (
...
export default CurriculumDifferentiationPageWrapper;