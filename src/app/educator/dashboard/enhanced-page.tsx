...
export default function EnhancedEducatorDashboard() {
  const router = typeof window !== "undefined" ? useRouter() : { push: () => {}, replace: () => {}, back: () => {}, pathname: "", query: {} };
  ...
  const studentAlerts = [
    ...
    { id: 2, student: 'James Wilson', issue: 'Attendance below 80% this month', priority: 'high', action: 'Timetable meeting' },
  ...
  const getActivityStatusIcon = (status): void => {
    switch(status) {
      case 'completed':
  ...
  const getResourceTypeIcon = (type): void => {
    switch(type) {
      case 'interactive':
  ...
  const renderSubscriptionWarning = (): void => {
              ...
              <Card className="card card-bordered bg-primary/5 hover:shadow-md transition-shadow">
                ...
                <CardContent className="card-body">
                  <Tabs defaultValue="generate" className="w-full">
                    ...
                    <TabsContent value="analyze" className="p-4 bg-muted/50 rounded-md">
                      <p className="text-sm mb-4">
                        Analyze student performance data to identify patterns and areas for improvement.
                      </p>
                      <Button className="w-full btn btn-primary" asChild>
                        <Link href="/educator/ai-assistant/analyze">
                          Analyze Data
                        </Link>
                      </Button>
                    </TabsContent>
