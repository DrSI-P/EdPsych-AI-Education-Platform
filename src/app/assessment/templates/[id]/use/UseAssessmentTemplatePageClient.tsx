...
export default function UseAssessmentTemplatePage() {
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        const response = await fetch(`/api/assessment/templates/${templateId}`, { cache: "no-store" });
        ...
        setError('An error occurred whilst fetching the template');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCreateAssessment = async () => {
    ...
    try {
      ...
      setError(err.message || 'An error occurred whilst creating the assessment');
    } finally {
      setSaving(false);
    }
  };
                ...
                <div className="space-y-2">
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e: any) => setAllowRetakes(e.target.checked)}
                    />
...
// Removed invalid standalone statement: UseAssessmentTemplatePage