...
export default function CreateAssessmentTemplatePage() {
  ...
  useEffect(() => {
    const fetchAssessments = async () => {
      try {
        const response = await fetch('/api/assessment?owned=true', { cache: "no-store" });
        ...
        setError('An error occurred whilst fetching your assessments');
      }
    };
  ...
  const handleSelectAssessment = (assessment): void => {
  ...
  const handleCreateTemplate = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/assessment/templates', {
        ...
        body: JSON.stringify(templateData, { cache: "no-store" }),
      });
      ...
      setError(err.message || 'An error occurred whilst creating the template');
    } finally {
      setLoading(false);
    }
  };

  const renderDetailsTab = (): void => {
    return (
  ...
  const renderSelectAssessmentTab = (): void => {
    return (
      <div className="space-y-6">
        <div className="mb-6">
...
// Removed invalid standalone statement: CreateAssessmentTemplatePage