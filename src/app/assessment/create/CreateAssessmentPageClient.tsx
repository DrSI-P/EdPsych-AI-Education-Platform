  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/assessment', {
        ...
        body: JSON.stringify(assessmentData, { cache: "no-store" }),
      });
      ...
      if (response.ok) {
        ...
        setError(data.message || 'An error occurred whilst creating the assessment');
      }
    } catch (err) {
      setError('An error occurred whilst creating the assessment');
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAiError(data.message || 'An error occurred whilst generating the assessment');
      }
    } catch (err) {
      setAiError('An error occurred whilst generating the assessment');
...
// Removed invalid standalone statement: CreateAssessmentPage