                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-grey-50 p-4 rounded-md">
                      <div>
                        ...
                        <p className="text-base font-medium">{responseDetails.user.name}</p>
                        <p className="text-sm text-grey-500">{responseDetails.user.email}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-grey-500">Submitted</p>
                        <p className="text-base font-medium">
                          {new Date(responseDetails.completedAt).toLocaleDateString()}
                        </p>
                        <p className="text-sm text-grey-500">
                          {new Date(responseDetails.completedAt).toLocaleTimeString()}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-grey-500">Score</p>
                        <p className="text-base font-medium">
                          {responseDetails.score || 0}/{totalPoints} ({(((responseDetails.score || 0) / totalPoints) * 100).toFixed(1)}%)
                        </p>
                      </div>
                    </div>
                          ...
                          <Card key={question.id} className={`border-l-4 ${
                            ...
                            <CardContent className="p-4">
                              <div className="flex justify-between items-start mb-2">
                                <div>
                                  <h5 className="text-base font-medium">Question {index + 1}</h5>
                                  <p className="text-sm text-grey-500">{question.type} • {question.points} points</p>
                                </div>
                                <div>
                                  {isCorrect === true && (
                                  ...
                                  {isCorrect === null && (
                                    <span className="px-2 py-1 text-xs font-semibold rounded-full bg-grey-100 text-grey-800">
                                      Not Graded
                                    </span>
                                  )}
                                </div>
                              ...
                              <div className="mb-3">
                                <p className="text-sm font-medium text-grey-500">Question:</p>
                                <p className="text-sm">{question.content}</p>
                              </div>
                              
                              <div className="mb-3">
                                <p className="text-sm font-medium text-grey-500">Answer:</p>
                                {answer ? (
                                  <div className="text-sm">
                                    {typeof answer.content === 'string' ? (
                                      <p className="whitespace-pre-wrap">{answer.content}</p>
                                    ) : (
                                      <p className="text-grey-500">Complex answer (view in assessment)</p>
                                    )}
                                  </div>
                                ) : (
                                  <p className="text-sm text-grey-500">No answer provided</p>
                                )}
                              </div>
                              
                              {answer?.feedback && (
                                <div>
                                  <p className="text-sm font-medium text-grey-500">Feedback:</p>
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="mt-2 text-grey-600">
          {assessment.title} - {assessment.subject} ({assessment.keyStage})
        </p>
      </div>
