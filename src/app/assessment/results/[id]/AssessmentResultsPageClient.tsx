...
export default function AssessmentResultsPage() {
  ...
  useEffect(() => {
    const fetchAssessment = async () => {
      try {
        const response = await fetch(`/api/assessment/${params.id}/results`, { cache: "no-store" });
        ...
        setError('An error occurred whilst fetching the assessment results');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchResponseDetails = async () => {
      ...
      try {
        const response = await fetch(`/api/assessment/response/${selectedResponse}`, { cache: "no-store" });
        ...
        setError('An error occurred whilst fetching the response details');
      } finally {
                ...
                {responseLoading ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex justify-end space-x-3">
                      <Button
                        variant="outline"
                        onClick={() => router?.push(`/assessment/mark/${responseDetails.id}`)}
                      >
                        Mark Response
                      </Button>
                      <Button
...
// Removed invalid standalone statement: AssessmentResultsPage