...
function AlignAssessmentContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (standardId) {
          const standardResponse = await fetch(`/api/curriculum/standards/${standardId}`, { cache: "no-store" });
          ...
          const assessmentsResponse = await fetch(`/api/assessment?subject=${standardData.subject}&keyStage=${standardData.keyStage}`, { cache: "no-store" });
          ...
          const alignedResponse = await fetch(`/api/curriculum/standards/${standardId}/assessments`, { cache: "no-store" });
        ...
        setError('An error occurred whilst fetching data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSaveAlignment = async () => {
    ...
    try {
      ...
      setError(err.message || 'An error occurred whilst saving the alignment');
    } finally {
      setSaving(false);
    }
  };
...
const AlignAssessmentPageWrapper = dynamic(
  () => Promise.resolve(AlignAssessmentPage),
  { 
    ssr: false,
    loading: () => (
...
export default AlignAssessmentPageWrapper;