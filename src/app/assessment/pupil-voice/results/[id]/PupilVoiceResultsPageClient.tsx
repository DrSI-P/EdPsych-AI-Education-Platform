  ...
  useEffect(() => {
    const fetchSurveyAndResponses = async () => {
      try {
        ...
        const surveyResponse = await fetch(`/api/assessment/pupil-voice/${surveyId}`, { cache: "no-store" });
        ...
        setError('An error occurred whilst fetching the survey data');
      } finally {
        setLoading(false);
      }
    };
...
// Removed invalid standalone statement: PupilVoiceResultsPage