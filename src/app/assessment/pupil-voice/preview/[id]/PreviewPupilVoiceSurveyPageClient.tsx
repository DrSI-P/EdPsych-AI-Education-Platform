...
export default function PreviewPupilVoiceSurveyPage() {
  ...
  useEffect(() => {
    const fetchSurvey = async () => {
      try {
        const response = await fetch(`/api/assessment/pupil-voice/${surveyId}`, { cache: "no-store" });
        ...
        if (data.status !== 'draft') {
          const responsesResponse = await fetch(`/api/assessment/pupil-voice/${surveyId}/responses`, { cache: "no-store" });
        ...
        setError('An error occurred whilst fetching the pupil voice survey');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handlePublishSurvey = async () => {
    try {
      ...
      setError('An error occurred whilst publishing the survey');
    } finally {
      setLoading(false);
    }
  };

  const handleEditSurvey = (): void => {
...
// Removed invalid standalone statement: PreviewPupilVoiceSurveyPage