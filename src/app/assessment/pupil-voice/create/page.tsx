  ...
  useEffect(() => {
    ...
    if (templateId) {
      ...
      const loadTemplate = async () => {
        try {
          const response = await fetch(`/api/assessment/pupil-voice/templates/${templateId}`, { cache: "no-store" });
          ...
          setError('An error occurred whilst loading the template');
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSaveSurvey = async (status: 'draft' | 'active' = 'draft') => {
    ...
    try {
      ...
      const response = await fetch('/api/assessment/pupil-voice', {
        ...
        body: JSON.stringify(surveyData, { cache: "no-store" }),
      });
      ...
      setError(err.message || 'An error occurred whilst saving the survey');
    } finally {
      setSaving(false);
    }
  };

  const renderQuestionEditor = (): void => {
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <textarea
            ...
            value={question.text}
            onChange={(e: any) => handleQuestionChange(activeQuestionIndex, 'text', e.target.value)}
        ...
        <div className="space-y-2">
          ...
          <select
            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            value={question.type}
            onChange={(e: any) => handleQuestionChange(activeQuestionIndex, 'type', e.target.value)}
          >
        ...
        <div className="flex items-center">
          <input
            ...
            onChange={(e: any) => handleQuestionChange(activeQuestionIndex, 'required', e.target.checked)}
          />
              ...
              {question.options.map((option, optionIndex) => (
                <div key={optionIndex} className="flex items-center">
                  <input
                    ...
                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    value={option}
                    onChange={(e: any) => handleOptionChange(activeQuestionIndex, optionIndex, e.target.value)}
...
// Original component
function CreatePupilVoiceSurveyPage() {
  return (
    <Suspense fallback={
      <div className="flex justify-center items-center min-h-screen">
        <Spinner size="lg" />
...
const CreatePupilVoiceSurveyPageWrapper = dynamic(
  () => Promise.resolve(CreatePupilVoiceSurveyPage),
  { 
    ssr: false,
    loading: () => (
...
export default CreatePupilVoiceSurveyPageWrapper;