        ...
        <div className="space-y-6">
          {loading ? (
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  <h3 className="text-lg font-semibold">Generate Personalized Study Plan</h3>
                </CardHeader>
                <CardContent>
                  <AIPrompt
                    placeholder="Describe what you want to focus on (e.g., preparing for upcoming assessments, improving in specific subjects)..."
                    systemPrompt={`You are an educational AI assistant helping ${studentData.name}, a ${studentData.year} student. Create a personalized study plan based on their current progress and the focus areas they mention. Their subject progress is: ${studentData.subjects.map(s => `${s.name}: ${s.progress}% (Current topic: ${s.currentTopic})`).join(', ')}. They have these upcoming assignments: ${studentData.assignments.filter(a => a.status === 'pending').map(a => `${a.title} (${a.subject}, due ${a.dueDate})`).join(', ')}. Create a structured, actionable study plan with specific activities, resources, and time management suggestions. Use UK English spelling and follow UK educational standards.`}
