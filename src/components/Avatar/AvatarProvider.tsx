...
// Dynamically import useSession to prevent SSR issues
const useSession = (): void => {
  try {
    // Only access next-auth in browser environment
    ...
    console.error('Error loading useSession:', error);
    return { data: null, status: 'loading' };
  }
};
...
const AVATAR_SCRIPTS = {
  ...
  'dashboard-professional': `Welcome to our Professional Collaboration Space - designed to facilitate the coordinated, evidence-based support that children and families deserve. Here we can share assessments and coordinate interventions whilst maintaining strict confidentiality.`,
  ...
  'curriculum-planning': `Effective curriculum planning requires balancing individual needs with curriculum requirements. Our planning tools make this complex task manageable whilst preserving the human elements of teaching that matter most.`,
...
export const usePageAvatar = (context: string, avatarId?: string, customScript?: string) => {
  ...
  try {
    ...
    console.error('Error accessing session in usePageAvatar:', error);
    // Fall back to student role if there's an error
  }
