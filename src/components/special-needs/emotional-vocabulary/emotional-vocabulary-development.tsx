...
const EmotionalVocabularyDevelopment = (): void => {
  ...
  const emotions = [
    ...
    },
    {
      ...
      ageGroups: ["early-years", "primary", "secondary"],
      examples: [
        "I felt calm whilst listening to soft music.",
    ...
    },
    {
      ...
      examples: [
        "I felt proud when I learnt to ride my bike.",
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setActivityHistory([
          ...
          { id: "h4", type: "emotion_explored", name: "Frustrated", date: "2025-05-10", notes: "Learnt new coping strategies" },
  ...
  const handleToggleFavorite = (emotionId): void => {
  ...
  const handleStartActivity = (activityId): void => {
  ...
  const handleStartQuiz = (quizId): void => {
  ...
  const handleDownloadResource = (resourceId): void => {
  ...
  const getFilteredEmotions = (): void => {
  ...
  const getFilteredActivities = (): void => {
  ...
  const getEmotionById = (id): void => {
    return emotions.find(emotion => emotion.id === id);
  };
  
  const getEmotionCategoryColor = (category): void => {
  ...
  const getIntensityLabel = (intensity): void => {
    switch (intensity) {
      case "low":
  ...
  const getAgeGroupLabel = (ageGroup): void => {
    switch (ageGroup) {
      case "early-years":
  ...
  const getProgressColor = (progress): void => {
    if (progress < 30) return "bg-red-500";
    if (progress < 70) return "bg-amber-500";
    return "bg-green-500";
  };
  
  const getMasteryLevelBadge = (level): void => {
    switch (level) {
      case "beginner":
  ...
  const getActivityTypeIcon = (type): void => {
    switch (type) {
              ...
              <>
                ...
                <TabsContent value="progress" className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <Card>
                      <CardHeader className="pb-2">
                        <CardTitle className="text-lg">Emotions Learnt</CardTitle>
                      </CardHeader>
                      <CardContent>
