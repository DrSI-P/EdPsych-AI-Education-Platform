...
const GuidedMindfulnessActivities = (): void => {
  ...
  const mindfulnessActivities = [
    {
      ...
      evidenceBase: "Supported by research from Oxford Mindfulness Center and recommended in NICE guidelines for stress management."
    },
    {
      id: "body-scan",
    ...
    },
    {
      ...
      description: "A colourful visualization practice especially suitable for children.",
      duration: 240, // 4 minutes in seconds
      category: "visualization",
      ageRange: "primary",
      difficulty: "beginner",
  ...
  const handleActivitySelection = (activity): void => {
  ...
  const handleVolumeChange = (value): void => {
    const newVolume = value[0];
    setVolume(newVolume);
    setIsMuted(newVolume === 0);
  };
  
  const handleToggleMute = async (): void => {
    setIsMuted(!isMuted);
  };
  ...
  const getActivityById = (id): void => {
    return mindfulnessActivities.find(activity => activity.id === id);
  };
  
  const getCategoryIcon = (category): void => {
    switch (category) {
      case 'breathing':
  ...
  const getAgeRangeLabel = (ageRange): void => {
    switch (ageRange) {
      case 'primary':
  ...
  const getDifficultyLabel = (difficulty): void => {
    switch (difficulty) {
      case 'beginner':
        return 'Beginner';
      case 'intermediate':
  ...
  const getDurationLabel = (seconds): void => {
