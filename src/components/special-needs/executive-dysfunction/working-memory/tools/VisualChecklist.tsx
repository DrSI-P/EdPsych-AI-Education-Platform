...
export default function VisualChecklist({
    ...
    const loadSavedLists = (): void => {
      try {
        ...
        if (savedListsData) {
          ...
          if (parsedData.currentListId && parsedData.lists.some((list: unknown) => list.id === parsedData.currentListId)) {
  ...
  const saveCurrentList = (): void => {
    try {
      ...
      const existingListIndex = parsedData.lists.findIndex((list: unknown) => list.id === currentListId);
