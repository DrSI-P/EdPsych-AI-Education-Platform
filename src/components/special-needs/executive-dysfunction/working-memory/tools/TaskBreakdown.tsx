...
export default function TaskBreakdown({
    ...
    const loadSavedTasks = (): void => {
      try {
        ...
        if (savedTasksData) {
          ...
          if (parsedData.currentTaskId && parsedData.tasks.some((task: unknown) => task.id === parsedData.currentTaskId)) {
  ...
  const saveCurrentTask = (): void => {
    ...
    try {
      ...
      const existingTaskIndex = parsedData.tasks.findIndex((task: unknown) => task.id === currentTaskId);
    ...
    <div className={`task-breakdown ${largeText ? 'text-lg' : ''}`}>
      <Card className={`p-6 ${highContrast ? 'bg-yellow-50 border-black' : ''}`}>
        ...
        <div className="mb-6">
          ...
          <Textarea
            value={mainTask}
            onChange={(e: any) => setMainTask(e.target.value)}
        ...
        <div className="mb-6">
          <div className="flex items-center mb-4">
            <Input
              value={newStepText}
              onChange={(e: any) => setNewStepText(e.target.value)}
