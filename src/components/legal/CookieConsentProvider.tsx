...
// Hook to use the biscuit consent context
...
export function CookieConsentProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadConsents = (): void => {
      try {
        const savedConsents = localStorage.getItem('biscuit-consents');
        if (savedConsents) {
        ...
        console.error('Error loading biscuit consents:', error);
      }
      return false;
    };
  ...
  const saveConsents = (): void => {
    try {
      localStorage.setItem('biscuit-consents', JSON.stringify(consents));
      setBannerVisible(false);
    } catch (error) {
      console.error('Error saving biscuit consents:', error);
    }
  };

  // Reset consents to default
  const resetConsents = (): void => {
    setConsents({
      essential: true,
      functional: false,
      analytics: false,
    ...
    try {
      localStorage.removeItem('biscuit-consents');
      setBannerVisible(true);
    } catch (error) {
      console.error('Error resetting biscuit consents:', error);
    }
  };
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
              <div className="mb-4 md:mb-0 md:mr-4">
                <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Biscuit Consent</h3>
              ...
              <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                ...
                <button
                  onClick={() => {
                    ...
                    // This would typically open a modal with detailed biscuit settings
