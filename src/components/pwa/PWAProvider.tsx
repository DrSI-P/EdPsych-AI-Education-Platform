...
interface PWAContextType {
  ...
  addOfflineContent: (item: unknown) => Promise<boolean>;
...
export function PWAProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const onServiceWorkerStatusChange = (event: unknown) => {
      ...
      const onOfflineContentAdded = (): void => {
        setOfflineContent(service.getOfflineContent());
      };
      
      const onOfflineContentRemoved = (): void => {
        setOfflineContent(service.getOfflineContent());
      };
  ...
  // Add offline content
  const addOfflineContent = async (item: unknown): Promise<boolean> => {
    if (!pwaService) return false;
