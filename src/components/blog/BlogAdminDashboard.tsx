    ...
    if (schedules.length === 0) {
      return (
        <div className="text-center py-8 text-muted-foreground">
          <p>No content schedules found. Create your first timetable to get started.</p>
          ...
          <Card key={timetable.id}>
            <CardHeader className="p-4 pb-2">
              <div className="flex justify-between items-start">
                ...
                <Badge variant={timetable.isActive ? 'default' : 'outline'}>
            ...
            <CardContent className="p-4 pt-2">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {timetable.frequency === 'weekly' 
                      ? `Weekly on ${getDayName(timetable.dayOfWeek)}`
                      : timetable.frequency === 'monthly'
                        ? `Monthly on day ${timetable.dayOfMonth}`
                        : 'Daily'}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {`${timetable.hour}:${timetable.minute.toString().padStart(2, '0')}`}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <BookOpen className="h-4 w-4 text-muted-foreground" />
                  <span className="text-sm">
                    {timetable.topicArea || 'Any topic'}
                    {timetable.keyStage ? ` (KS${timetable.keyStage})` : ''}
                  </span>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button variant="outline" size="sm" className={timetable.isActive ? 'text-destructive' : ''}>
