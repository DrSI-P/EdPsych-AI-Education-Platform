...
export function NaturalLanguageConceptExplainer({
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const errorMessage = {
        id: `error-${Date.now()}`,
        type: 'error',
        content: 'Sorry, I encountered an error whilst generating an explanation. Please try again.',
        timestamp: new Date().toISOString()
      };
  ...
  const generateExplanation = async (query) => {
    try {
      ...
      const prompt = `
        ...
        4. Connections to previously learnt concepts
  ...
  const generateMathExplanation = (query, style, mode): void => {
    ...
    if (style === LearningStyle.VISUAL) {
      return `${baseContent}
        ...
        The coloured squares represent the parts we're considering, whilst the empty squares represent the remaining parts of the whole.
  ...
  const generateScienceExplanation = (query, style, mode): void => {
    // Base content adjusted for proficiency level
    let baseContent = '';
  ...
  const generateFollowUpQuestions = (query, explanation): void => {
  ...
  const handleFormatChange = (format): void => {
  ...
  const handleFeedback = (explanationId, rating): void => {
  ...
  const handleFollowUpClick = (question): void => {
    setUserQuery(question);
  };
  
  // Handle related concept click
  const handleRelatedConceptClick = (concept): void => {
    setUserQuery(`Can you explain ${concept.name}?`);
  };
  
  // Helper function to capitalize first letter
  const capitalizeFirstLetter = (string): void => {
    return string.charAt(0).toUpperCase() + string.slice(1);
  };
  
  // Render explanation
  const renderExplanation = (explanation): void => {
