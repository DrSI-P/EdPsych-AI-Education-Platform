...
export function AITutoringSession({
  ...
  const formatDuration = (seconds): void => {
  ...
  const generateTutorResponse = async (userQuery) => {
    ...
    try {
      ...
      const prompt = `
        You are an educational tutor specialising in ${subject} for UK Key Stage ${keyStage}.
      ...
      const errorMessage = {
        id: `error-${Date.now()}`,
        sender: 'system',
        content: 'Sorry, I encountered an error whilst generating a response. Please try again.',
        timestamp: new Date().toISOString(),
        type: 'error'
      };
  ...
  const generateMathExplanation = (query, style): void => {
    ...
    // Kinesthetic learner explanation
      ...
      return `
        ...
        3. Now look at both papers together - you have 3 coloured parts on the first and 1 coloured part on the second
        4. Together, that's 4 coloured parts out of 8 total parts, or 4/8 (which simplifies to 1/2)
  ...
  const generateMathExample = (style): void => {
  ...
  const generateSuggestedQuestions = (tutorResponse, conceptDetected): void => {
  ...
  // Generate session summary
  const generateSessionSummary = (): void => {
  ...
  // Update session progress
  const updateSessionProgress = (): void => {
  ...
  const handleSuggestedQuestionClick = (question): void => {
  ...
  // Render message
  const renderMessage = (message): void => {
    switch (message.type) {
      case 'text':
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-neutral-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <Input 
                value={sessionGoal} 
                onChange={(e: any) => setSessionGoal(e.target.value)} 
