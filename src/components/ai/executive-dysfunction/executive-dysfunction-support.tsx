...
export default function ExecutiveDysfunctionSupport({
  ...
  const [dailySchedule, setDailySchedule] = useState([
    { time: '08:00', activity: 'Morning routine', icon: '☀️', color: 'bg-yellow-100' },
    { time: '09:00', activity: 'Focus work session', icon: '💻', color: 'bg-blue-100' },
    { time: '10:30', activity: 'Break', icon: '☕', color: 'bg-green-100' },
    { time: '11:00', activity: 'Meeting/Collaboration', icon: '👥', color: 'bg-purple-100' },
    { time: '12:00', activity: 'Lunch', icon: '🍽️', color: 'bg-orange-100' },
    { time: '13:00', activity: 'Afternoon tasks', icon: '📋', color: 'bg-indigo-100' },
    { time: '15:00', activity: 'Creative work', icon: '🎨', color: 'bg-pink-100' },
    { time: '16:30', activity: 'Review & planning', icon: '📊', color: 'bg-gray-100' },
    { time: '17:00', activity: 'End of day', icon: '🏠', color: 'bg-amber-100' }
  ]);
  
  // Visual reminders state
  const [visualReminders, setVisualReminders] = useState([
    { id: '1', title: 'Take medication', icon: '💊', color: 'bg-red-100 text-red-800', time: '08:00', active: true },
    { id: '2', title: 'Drink water', icon: '💧', color: 'bg-blue-100 text-blue-800', repeat: 'hourly', active: true },
    { id: '3', title: 'Stretch break', icon: '🧘', color: 'bg-green-100 text-green-800', time: '14:00', active: true },
    { id: '4', title: 'Review tomorrow\'s tasks', icon: '📝', color: 'bg-purple-100 text-purple-800', time: '20:00', active: false }
  ]);
  ...
  const calculateTaskProgress = (task): void => {
  ...
  const renderTaskManagement = (): void => {
    return (
  ...
  const renderTimeManagement = (): void => {
    return (
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {dailySchedule.map((item, index) => {
                  ...
                  itemTime.setHours(parseInt(hours), parseInt(minutes), 0);
                  
                  const isPast = currentTime > itemTime;
                  const isCurrent = index < dailySchedule.length - 1 &&
                    currentTime >= itemTime &&
                    currentTime < (() => {
                      const nextTime = new Date();
                      if (dailySchedule[index + 1]) {
                        const [nextHours, nextMinutes] = dailySchedule[index + 1].time.split(':');
                        nextTime.setHours(parseInt(nextHours), parseInt(nextMinutes), 0);
                      } else {
                  ...
                  return (
                    <div
                      ...
                      } ${isPast && !isCurrent ? 'opacity-60' : ''} ${item.color}`}
                    >
              ...
              <div className="flex gap-2 mt-4">
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    const currentBlock = dailySchedule.find((item, index) => {
                      const currentTime = new Date();
                      const itemTime = new Date();
                      const [hours, minutes] = item.time.split(':');
                      itemTime.setHours(parseInt(hours), parseInt(minutes), 0);
                    ...
                    toast({
                      ...
                      description: currentBlock ?
                        `${currentBlock.icon} ${currentBlock.activity} (${currentBlock.time})` :
                        "No scheduled activity at this time",
                    });
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() => {
                    toast({
                      ...
                      description: "Schedule editing will be available in settings.",
                    });
                  }}
                >
              ...
              <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-900 rounded-md">
                ...
                <Progress
                  value={(() => {
                    ...
                    const elapsedMinutes = (now.getTime() - dayStart.getTime()) / 60000;
  ...
  const renderWorkingMemory = (): void => {
    return (
      <div className="space-y-6">
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-4">
                    ...
                    <div className="flex-shrink-0 ml-3">
                      <Switch
                        ...
                        onCheckedChange={(checked) => {
                          setVisualReminders(visualReminders.map(r =>
                            r.id === reminder.id ? { ...r, active: checked } : r
                          ));
                          toast({
                            title: checked ? "Reminder activated" : "Reminder deactivated",
                            description: `${reminder.title} has been ${checked ? 'activated' : 'deactivated'}.`,
                          });
                        }}
                      />
                    </div>
              ...
              <div className="border-t pt-4">
                <h4 className="text-sm font-medium mb-3">Add New Reminder</h4>
                <div className="space-y-3">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" size="icon">
                          <span className="text-lg">{newReminder.icon}</span>
                        </Button>
                      </PopoverTrigger>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setVisualReminders([...visualReminders, reminder]);
                      setNewReminder({
                        title: '',
                        icon: '⭐',
                        color: 'bg-blue-100 text-blue-800',
                      ...
                      toast({
                        ...
                        description: `${reminder.title} has been added to your visual reminders.`,
                      });
                    }}
                  >
              ...
              <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-md">
                ...
                <ul className="text-sm space-y-1">
                  <li>• Place visual cues in locations you frequently look at</li>
                  <li>• Use bright colors and clear icons for better visibility</li>
                  <li>• Set regular reminders for important daily tasks</li>
                  <li>• Keep reminders simple and action-oriented</li>
                </ul>
              </div>
            </div>
          </CardContent>
  ...
  const renderEmotionalRegulation = (): void => {
    return (
      <div className="space-y-6">
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">
                  Allow customisation of focus timer durations
                </p>
              </div>
              <Switch 
