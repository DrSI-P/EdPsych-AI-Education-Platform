...
export default function AutomatedProgressReportGeneration() {
  ...
  const generateReports = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "An error occurred whilst generating the reports. Please try again.",
        variant: "destructive"
      });
    } finally {
  ...
  const generateOverallComments = (student: unknown, subjectReports: any[], style: string) => {
    ...
    switch (style) {
      ...
      default:
        ...
        comment += `Particular strengths are evident in ${subjectReports.sort((a, b) => b.currentScore - a.currentScore)[0].subject}, whilst ${subjectReports.sort((a, b) => a.currentScore - b.currentScore)[0].subject} presents opportunities for further development. `;
        ...
        comment += behaviorRatio > 0.7 ? `Behavior has been positive throughout the term. ` : `Some improvements in classroom behavior would enhance learning experiences. `;
  ...
  const editReport = (): void => {
    ...
    currentReport.subjects.forEach((subject: unknown) => {
  ...
  // Save edited report
  const saveEditedReport = (): void => {
    if (!currentReport) return;
    
    // Update subject comments
    const updatedSubjects = currentReport.subjects.map((subject: unknown) => ({
      ...subject,
  ...
  const saveReport = (report: unknown) => {
  ...
  const printReport = (): void => {
    ...
    // In a real implementation, this would open the print dialogue
    // For now, we'll just show a toast
    toast({
      title: "Print dialogue",
      description: "The print dialogue would normally open here.",
      variant: "default"
    });
  };
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {Math.round(report.subjects.reduce((sum: number, s: unknown) => sum + s.currentScore, 0) / report.subjects.length)}%
                        </span>
                      </div>
                    </div>
        ...
        <TabsContent value="view" className="space-y-6">
          {!currentReport ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* Subject Reports */}
              {currentReport.subjects.map((subject: unknown) => (
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {currentReport.includeBehavior && (
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant="outline">{subject.behavior}</Badge>
                      </div>
                    )}
                  </CardContent>
                </Card>
