...
export default function AIAssessmentGeneratorClient() {
  ...
  const handleGenerateAssessment = async () => {
    ...
    try {
      ...
      setError(err.message || 'An error occurred whilst generating the assessment');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleSaveAssessment = async () => {
    ...
    try {
      ...
      setError(err.message || 'An error occurred whilst saving the assessment');
    } finally {
      setLoading(false);
    }
  };

  const handleToggleQuestionType = (type): void => {
              ...
              <Form className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                    value={keyStage}
                    onChange={(e: any) => setKeyStage(e.target.value)}
                  >
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                    value={questionCount}
                    onChange={(e: any) => setQuestionCount(parseInt(e.target.value) || 10)}
                  />
                </div>
              </Form>
            </CardContent>
