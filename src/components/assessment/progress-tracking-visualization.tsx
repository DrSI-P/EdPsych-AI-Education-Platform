...
export function ProgressTrackingVisualization({
            ...
            <div className="h-[100px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip 
                    formatter={(value: unknown) => [`${value} areas`, 'Count']}
                    labelFormatter={() => ''}
                  />
                </PieChart>
              </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={progressOverTimeData}>
                ...
                <Tooltip 
                  ...
                  formatter={(value: unknown) => [`${Math.round(value)}%`, 'Score']}
                />
                <Legend />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart outerRadius={90} data={subjectAverages}>
                  ...
                  <Radar
                    ...
                    fillOpacity={0.6}
                  />
                  <Tooltip formatter={(value: unknown) => `${Math.round(value)}%`} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
  ...
  const renderSpacedRepetitionTab = (): void => {
    ...
    // Simulate spaced repetition timetable
    filteredProgressRecords.forEach(record => {
      if (record.knowledgeAreaId) {
        const ka = knowledgeAreas.find(ka => ka.id === record.knowledgeAreaId);
        if (!ka) return;
        
        // Only timetable repetition for areas that need improvement
        if (
    ...
    return (
      <div className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>Spaced Repetition Timetable</CardTitle>
            <CardDescription>
              Optimized review timetable based on learning science
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              {sortedDates.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Complete more assessments to generate a spaced repetition timetable
                  </p>
                </div>
              )}
            </div>
