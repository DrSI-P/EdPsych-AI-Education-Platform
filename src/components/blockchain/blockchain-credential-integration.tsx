...
const PortfolioIntegration = async ({ credential, onAddToPortfolio }): void => {
    ...
    <div className="space-y-4 py-4">
      <div>
        ...
        <select
          ...
          onChange={(e: any) => setPortfolioSection(e.target.value)}
        >
...
const CPDIntegration = ({ credential, onLinkToCPD }): void => {
    ...
    <div className="space-y-4 py-4">
      <div>
        ...
        <select
          ...
          className="w-full mt-1 flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
          value={selectedCPD}
          onChange={(e: any) => setSelectedCPD(e.target.value)}
        >
...
const BlockchainCredentialIntegration = ({ credential, onClose }): void => {
  const [activeTab, setActiveTab] = useState('portfolio');
  const { toast } = useToast();
  
  const handleAddToPortfolio = (credential, section): void => {
  ...
  const handleLinkToCPD = (credential, activity): void => {
