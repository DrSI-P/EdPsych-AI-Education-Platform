...
export function ContentCreator({
  ...
  const handleContentVariantChange = (index: number, field: keyof ContentVariant, value: unknown) => {
    setContent(prev => {
  ...
  const handleContentDataChange = (variantIndex: number, field: keyof ContentData, value: unknown) => {
    setContent(prev => {
      const newVariants = [...(prev.content || [])];
  ...
  const renderCurriculumStandardsTab = (): void => {
        ...
        <div className="space-y-4">
          {content.learningObjectives.map((objective, index) => (
            <div key={index} className="flex items-start gap-2">
              <Textarea
                value={objective}
                onChange={(e: any) => handleArrayFieldChange('learningObjectives', index, e.target.value)}
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={(e: any) => handleContentDataChange(index, 'body', e.target.value)}
          ...
          <div className="space-y-2">
            {content.prerequisites?.map((prerequisite, index) => (
              <div key={index} className="flex items-center gap-2">
                <Input
                  value={prerequisite}
                  onChange={(e: any) => handleArrayFieldChange('prerequisites', index, e.target.value)}
          ...
          <div className="space-y-2">
            {content.relatedContent?.map((related, index) => (
              <div key={index} className="flex items-center gap-2">
                <Input
                  value={related}
                  onChange={(e: any) => handleArrayFieldChange('relatedContent', index, e.target.value)}
        ...
        <div className="space-y-2">
          {content.assessments?.map((assessment, index) => (
            <div key={index} className="flex items-center gap-2">
              <Input
                value={assessment}
                onChange={(e: any) => handleArrayFieldChange('assessments', index, e.target.value)}
