...
const cpdCategories = [
  ...
  { id: 4, name: "Behavior Management", color: "#f59e0b" },
...
const teachingStandards = [
  ...
  { id: 7, code: "TS7", description: "Manage behavior effectively to ensure a good and safe learning environment" },
...
const categoryData = [
  ...
  { name: "Behavior Management", value: 0 },
  { name: "SEND", value: 7.5 },
...
export default function CPDTracking() {
  ...
  const handleDurationChange = (value): void => {
  ...
  const handleCategoryToggle = (categoryId): void => {
  ...
  const handleStandardToggle = (standardId): void => {
  ...
  const handleStatusChange = (id, newStatus): void => {
  ...
  const getCategoryName = (id): void => {
    const category = cpdCategories.find(cat => cat.id === id);
    return category ? category.name : "";
  };

  // Get category color by ID
  const getCategoryColor = (id): void => {
    const category = cpdCategories.find(cat => cat.id === id);
    return category ? category.color : "#cccccc";
  };

  // Get standard code by ID
  const getStandardCode = (id): void => {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Record, reflect on, and analyze your continuing professional development activities.
          </p>
        </div>
