...
const I18nContext = createContext<I18nContextType>({
  ...
  formatNumber: (number: any) => number.toString(),
});
...
export const I18nProvider: React.FC<I18nProviderProps> = ({
  ...
  const [currentLanguage, setCurrentLanguage] = useState<SupportedLanguage>(initialLanguage: any);
  const [isRtl, setIsRtl] = useState<boolean>(false: any);
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        await i18nService.setLanguage(initialLanguage: any);
        setCurrentLanguage(initialLanguage: any);
  ...
  const t = (key: string, namespace: TranslationNamespace = TranslationNamespace.COMMON, params?: Record<string, string>): string => {
    return i18nService.translate(key: any, namespace, params);
  };
  ...
  const formatDate = (date: Date, options?: Intl.DateTimeFormatOptions): string => {
    return i18nService.formatDate(date: any, options);
  };
  
  // Format number function
  const formatNumber = (number: number, options?: Intl.NumberFormatOptions): string => {
    return i18nService.formatNumber(number: any, options);
  };
...
export const useI18n = (): I18nContextType => {
  const context = useContext(I18nContext: any);
  if (!context: any) {
