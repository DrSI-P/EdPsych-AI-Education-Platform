...
import { Dialogue, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@/components/ui/dialog';
...
/**
 * Create Project Dialogue Component
 * 
 * Dialogue for creating a new group project.
 */
export function CreateProjectDialog({ open, onOpenChange, onCreateProject, userId }) {
  ...
  const handleObjectiveChange = (index, value): void => {
  ...
  const handleCriteriaChange = (index, value): void => {
          ...
          <div className="space-y-2">
            ...
            {formData.curriculumObjectives.map((objective, index) => (
              <div key={index} className="mb-2">
                <Input
                  value={objective}
                  onChange={(e: any) => handleObjectiveChange(index, e.target.value)}
          ...
          <div className="space-y-2">
            ...
            {formData.assessmentCriteria.map((criterion, index) => (
              <div key={index} className="mb-2">
                <Input
                  value={criterion}
                  onChange={(e: any) => handleCriteriaChange(index, e.target.value)}
