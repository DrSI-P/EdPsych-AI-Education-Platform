...
export interface DFEComplianceService {
  validateCurriculumAlignment: (content: unknown, keyStage: UKKeyStage) => Promise<{
    isAligned: boolean;
  ...
  checkAgeAppropriateness: (content: unknown, targetAgeRange: { min: number; max: number }) => Promise<{
  ...
  validateAccessibility: (content: unknown) => Promise<{
  ...
  checkInclusivity: (content: unknown) => Promise<{
    isInclusive: boolean;
...
export interface DataProtectionService {
  classifyData: (data: unknown) => Promise<{
    categories: string[];
