...
export interface DataProtectionService {
  ...
  applyDataMinimization: (data: any, purpose: string) => Promise<{
    minimizedData: any;
    removedFields: string[];
  }>;
  
  applyPseudonymization: (data: any, fields: string[]) => Promise<{
    pseudonymizedData: any;
    mappingKey: string;
  }>;
  
  applyAnonymization: (data: any) => Promise<{
    anonymizedData: any;
    anonymizationLevel: 'partial' | 'complete';
    reidentificationRisk: 'low' | 'medium' | 'high';
  }>;
  
  encryptData: (data: any, level: 'standard' | 'high') => Promise<{
