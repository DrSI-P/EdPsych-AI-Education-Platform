...
export async function generateBlogPost({
  ...
  try {
    const completion = await openai.chat.completions.create({
      model: "gpt-4",
      messages: [
        { role: "system", content: "You are an expert educational content creator specialising in UK educational psychology and curriculum." },
...
/**
 * Timetable a blog post for publication
 */
...
export async function generateBlogPostIdeas(count: number = 5, topics?: string[]): Promise<BlogPostIdea[]> {
  ...
  try {
    const completion = await openai.chat.completions.create({
      model: "gpt-4",
      messages: [
        { role: "system", content: "You are an expert educational content strategist specialising in UK educational psychology and curriculum." },
        { role: "user", content: prompt }
      ],
      response_format: { type: "json_object" }
    });
