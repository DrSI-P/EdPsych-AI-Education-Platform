...
/**
 * Timetable a new meeting between a parent and an educator
 * @param meeting The meeting to timetable
...
export async function scheduleMeeting(meeting: Omit<ParentEducatorMeeting, 'id'>): Promise<ParentEducatorMeeting> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to timetable meeting: ${response.statusText}`);
  }
...
/**
 * Fetch a child's behavior records
 * @param childId The ID of the child
 * @param params Optional query parameters
 * @returns Promise resolving to an array of behavior records
 */
export async function getChildBehaviourRecords(
  ...
  const url = `/api/parent-portal/children/${childId}/behavior${
    queryParams.toString() ? `?${queryParams.toString()}` : ''
  }`;
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch child behavior records: ${response.statusText}`);
  }
