...
export class ApiWebhookService {
   ...
   */
  public async triggerEvent(tenantId: string, event: string, payload: unknown): Promise<void> {
    try {
   ...
   */
  public generateSignature(payload: unknown, secret: string): string {
  ...
  private async sendWebhookRequest(url: string, data: unknown, signature: string): Promise<any> {
