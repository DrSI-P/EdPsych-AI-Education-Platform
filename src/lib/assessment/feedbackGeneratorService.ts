...
export class FeedbackGeneratorService implements FeedbackGenerator {
   ...
   */
  generateQuestionFeedback(question: Question, response: unknown, correct: boolean): string {
  ...
  private generateOverallFeedback(result: AssessmentResult): string {
    ...
    if (percentage >= 90) {
      ...
      return `You've passed with a score of ${percentage.toFixed(1)}%. Whilst you've demonstrated understanding of some key concepts, there are important areas to improve, particularly in ${result.analytics.areasForImprovement[0] || 'several topics'}.`;
    } else {
