...
export class ProgressiveWebAppService {
  ...
  private installPromptEvent: unknown = null;
   ...
   */
  public addToSyncQueue(id: string, data: unknown): void {
   ...
   */
  private async syncItem(id: string, item: unknown): Promise<boolean> {
    try {
  ...
  public async registerPeriodicSync(): Promise<boolean> {
    ...
    try {
      ...
      if (this.serviceWorkerRegistration && 'periodicSync' in this.serviceWorkerRegistration) {
        const periodicSync = async (this.serviceWorkerRegistration as any).periodicSync;
   ...
   */
  public generateManifest(): unknown {
    return {
