...
find src -type f \( -name "*.tsx" -o -name "*.ts" \) -exec grep -l '<Alert.*type=' {} \; | while read file; do
...
find src -type f \( -name "*.tsx" -o -name "*.ts" \) -exec grep -l '<Spinner.*size="large"' {} \; | while read file; do
...
find src -type f \( -name "*.tsx" -o -name "*.ts" \) -exec grep -l '<Tabs.*tabs=' {} \; | while read file; do
...
find src -type f \( -name "*.tsx" -o -name "*.ts" \) -exec grep -l '<[a-zA-Z]' {} \; | while read file; do
...
find src -type f \( -name "*.tsx" -o -name "*.ts" \) -exec grep -l "import.*Tabs.*from" {} \; | while read file; do
