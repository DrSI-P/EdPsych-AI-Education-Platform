#!/usr/bin/env python3
"""
Final manual fix script for learning-utils.tsx TypeScript errors
This script uses a highly targeted approach to fix the specific syntax errors in the learning utils file
by addressing the complex React component structure issues and escaping issues.
"""
...
def fix_learning_utils_final(file_path):
    try:
        ...
        pattern = r'\} = \(\) => \{'
        replacement = r'const useLearningContent = () => {'
        content = re.sub(pattern, replacement, content)
        
        # Fix useState with escaped quotes
        content = re.sub(
            r"useState<string>\(\\\'",
            r"useState<string>('",
            content
        ...
        # Fix the second problematic hook declaration
        content = re.sub(
            r'\} = \(\) => \{',
            r'const useAdaptiveLearning = () => {',
            content
        )
...
if __name__ == "__main__":
    ...
    fix_learning_utils_final(file_path)
